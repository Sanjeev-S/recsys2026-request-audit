...
- ``adjudication``: blinded pairwise adjudication over the annotation set from
  adjudication_set.py. The model sees the visible user
  ...
  score_adjudication.py against the withheld key.

- ``strict-review``: unblinded strict review over the changed-rows packet from
  build_changed_rows.py. The model sees the message, the
...
def main(argv: list[str] | None = None) -> None:
    ...
    protocol = {
        ...
        "blinding": (
            "candidates A/B randomized by adjudication_set.py (key withheld from the model)"
